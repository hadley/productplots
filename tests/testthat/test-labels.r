...
test_that("labelling levels identified corrected", {
  ...
  expect_identical(find_col_level(a), 1)
  expect_identical(find_row_level(a), 1)

  b <- prodcalc(happy, ~ finrela | degree, c("vbar", "hspine"), na.rm = T)
  expect_identical(find_col_level(b), 1)
  expect_identical(find_row_level(b), 2)

  c <- prodcalc(happy, ~ finrela | degree, c("vbar", "vspine"), na.rm = T)
  expect_identical(find_col_level(c), NA_real_)
  expect_identical(find_row_level(c), 1)
})
